...
19 atomic tasks split across backend bootstrap (tasks 1–9), local-dev helper (10–11), frontend shared infrastructure (12–15), styling (16), page wiring (17–18), and steering housekeeping (19). Backend can be implemented in parallel with frontend layers; the only hard sequencing is **task 14 (sync.js network half) before task 15 (user-code.js)** and **all frontend code before tasks 17–18 (page wiring)**.

Each task touches 1–3 files and is independently verifiable: backend tasks via `curl` against `npm run dev` once tasks 1–11 complete; frontend tasks by visiting `http://localhost:8000` and exercising in DevTools. End-to-end verification + Playwright happens in spec-test / spec-review, not here.
...
- All colors via existing tokens in `conventions/colors.md`. The single new token `--overlay-tint` is added in task 16 with documentation in `conventions/colors.md`.
- Dependency budget: exactly two (`hono`, `@libsql/client`). No devDependencies.

## Tasks

- [X] 1. Create `package.json` + `.env.example` + extend `.gitignore`
    ...
    - Purpose: Define the manifest. (Lockfile generated in task 2.)
...
- [X] 2. Generate + commit `package-lock.json`
    ...
    - Commit `package-lock.json`. Do NOT commit `node_modules/` (gitignored in task 1).
...
- [X] 3. Create initial Turso migrations
...
- [X] 4. Idempotent migration runner `scripts/migrate.mjs`
...
- [X] 6. Rate-limit module `server/rate-limit.js`
...
- [X] 7. Hono app skeleton + CORS + structured-log wrapper + `/api/health` in `server/app.js`
...
- [X] 8. State routes (`GET / PUT / DELETE /api/state`) in `server/app.js`
    - File: `server/app.js` (extends task 7)
    ...
    - Purpose: All real API behavior. With task 7's middleware in place, every path through these routes (success, 400, 413, 429) ends in a structured log line and the correct status. Verifiable independently with `curl -X PUT -d '{"code":"test123","data":{},"updatedAt":1}' -H 'Content-Type: application/json' http://localhost:8000/api/state` etc.
    - _Leverage: task 7's middleware (CORS, log wrapper, `c.set` slots); `getDb`, `validateCode`, `payloadByteLength`, `checkAndIncrement` from tasks 5 and 6_
    - _Requirements: R3#3 (PUT 204/400/413), R7#1 (schema use), R9#3 (DELETE idempotent), R10#1 (413), R10#2 (429 + Retry-After)_

- [X] 9. EdgeOne Pages Functions entry shim `functions/api/[[path]].js`
...
- [X] 10. Local dev shim — env loader + Hono fetch bridge in `scripts/dev.mjs`
    ...
    - ESM. Sketch:
      ...
      - For non-API paths, defer to task 11's static-file branch (in task 10, return a 501 placeholder so the file is still runnable). `server.listen(process.env.PORT || 8000)`.
...
- [X] 11. Local dev shim — static file fallback in `scripts/dev.mjs`
    - File: `scripts/dev.mjs` (extends task 10)
    ...
    - _Leverage: task 10's `http.createServer` skeleton; Node stdlib `fs.readFile`, `path.extname`, `path.join`_
    - _Requirements: (dev convenience supporting R8 verification)_

- [X] 12. Extend `js/shared.js` with cookie-double-write for numbers, silent variants, prefix warning, clock helper + swap `recordActiveDay` call site
...
- [X] 13. Frontend sync — storage layer in `js/sync.js`
    ...
    - Add module-level state at top: `var _oversizeBlocked = false; var _initialSyncBlocking = false; var _lastSyncAt = null; var _lastError = null; var _debounceTimer = null;`. (Network half in task 14 will reference these.)
    ...
    - _Leverage: `MyStar.readJSON / writeJSON / readNumber / writeNumber / writeJSONSilent / writeNumberSilent / touchLocalUpdatedAt` from task 12; `_writeCookieStr`'s SameSite contract_
...
- [X] 14. Frontend sync — network + orchestration in `js/sync.js`, plus `unit-enhance.js` swap
    ...
    - In `js/sync.js`, add to the existing IIFE (built in task 13):
      ...
      - **`__deleteCloudData(cb)`** — used by R9 flow in task 15. Reads current code via `getUserCode()`, calls `_xhrJson('DELETE', '/api/state', { code }, cb)`. Treats both 204 and any 4xx-on-missing as success (idempotent per R9#3).
      ...
      - **Exports added in this task** — these names are the contract task 15 depends on, do NOT rename:
        - Public on `MyStar`: `syncFromServer`, `syncToServer`, `flushPendingSync`, `getSyncStatus`.
        - Private hooks: `__notifyChange`, `__xhrJson`, `__deleteCloudData`. (`__hasLocalUserState`, `__applyServerBlob` already exported in task 13.)
        - Callback slot left for task 15 to set: `__onSyncStatus`.
    - Purpose: The sync engine. After this task lands, sync is fully functional end-to-end except for the UI (task 15) and page wiring (tasks 17–18).
    - _Leverage: storage layer from task 13; `_writeCookieStr` from shared.js (task 12); design "Frontend: `js/sync.js`" subsection_
...
- [X] 15. Frontend identity UI `js/user-code.js`
    ...
    - ES5 IIFE. Implements design "Frontend: `js/user-code.js`" subsection:
      ...
      - **Probe flow** on "开始" tap: validate against `/^[a-zA-Z0-9_\-]{6,16}$/`; show inline error if invalid. On valid input, **do not commit the code yet** — call `_probeIdentity(typedCode)` which:
        1. `MyStar.__xhrJson('GET', '/api/state?code=' + encodeURIComponent(typedCode), null, cb)` — exported by task 14, signature `cb(err, status, parsedJson)`, 8 s timeout. The single canonical XHR helper; do NOT inline a duplicate.
    ...
    - Purpose: All identity UX in one place; sync.js (tasks 13–14) stays free of DOM concerns.
    - _Leverage: `MyStar.addTapListener` (shared.js:60), `MyStar.timeAgo` (shared.js:82); identity + apply hooks from sync.js (tasks 13–14)_
...
- [X] 16. Add `--overlay-tint` token + sync-UI styles in `style/shared.css` + update `conventions/colors.md`
...
- [X] 17. Wire `index.html` + `review/index.html`
    ...
    - Do NOT move any existing `<script>` tag. Do NOT alter the existing inline `M.recordActiveDay()` boot — it now flows through `writeJSONSilent` automatically (task 12 swap), and runs FIRST (because its DCL handler is registered before the new tail script's handler).
...
- [X] 18. Surgical edits + script additions in `units/book1/u3/index.html` + `u6/index.html`
...
- [X] 19. Steering updates: `product.md`, `tech.md`, `structure.md`
